...
import { uploadOnCloudinary , deleteOnCloudinary} from "../utils/cloudinary.js"
 ...
 const publishAVideo = asyncHandler (async (req, res) => {
    ...
    if (!videoFile) {
        throw new ApiError(400, "vido file is not found")
    }
    if (!thumbnail) {
    ...
    if (!videoId) {
        throw new ApiError(501, "videoUpload failed please try again !!!");
    }
 ...
 const updateVideo = asyncHandler (async (req, res) => {
    ...
    const updateVideos = await Video.findByIdAndUpdate(
        videoId,
        {
            $set : {
                title,
    ...
    .status(200)
    .json(
        new ApiResponse(200, updateVideos, "Video updated successfully")
    )
 })
 ...
 const togglePublishStatus = asyncHandler (async (req, res) => {
    ...
    if (!video) {
     ...
     if (video?.owner.toString() !== req.user?._id.toString()) {
        throw new ApiError(400, "You can't toogle status  as you are not the owner")
    }
